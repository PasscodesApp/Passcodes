        ...
        lifecycleScope.launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    val lastUpdatedAt = DateTimeUtils.getRelativeDays(
                        passwordEntity.updatedAt.orEmpty(),
                        "yyyy-MM-dd HH:mm:ss"
                    )

                    binding.tvDomain.text =
                        "${getString(R.string.domain_prefix)}  ${passwordEntity.domain}"
                    binding.tvUsername.text =
                        "${getString(R.string.username_prefix)}  ${passwordEntity.username}"
                    binding.tvPassword.text =
                        "${getString(R.string.password_prefix)}  ${passwordEntity.password}"
                    binding.tvNotes.text =
                        "${getString(R.string.notes_prefix)}  ${passwordEntity.notes}"
        ...
        binding.copyPasswordBtn.setOnClickListener {
            ...
            val confirmDialog = AlertDialog.Builder(this@ViewPasswordActivity)
                ...
                .setPositiveButton(R.string.confirm_dialog_button_text) { dialog, which ->
                    ...
                    val clip: ClipData =
                        ClipData.newPlainText(passwordEntity.username, passwordEntity.password)
            ...
            try {
                ...
                withContext(Dispatchers.Main) {
                    if (rowsDeleted > 0) {
                        ...
                        Toast.makeText(
                            this@ViewPasswordActivity,
                            getString(R.string.something_went_wrong_msg) + ": Password not found for deletion or no rows affected.",
                            Toast.LENGTH_SHORT
                        ).show()
                        finish()
                    }
